...
# 統計関連のCRUD操作をインポート
from crud.simple_statistics import read_user_statistics, read_overall_statistics, create_user_statistics, update_user_statistics
...
def post_achievement(user_id: uuid.UUID, money_saved: float, co2_reduction: float, db: Session = Depends(get_db)):
    stats = ensure_user_statistics_exists(db, user_id)

    stats = update_user_statistics(db, user_id, money_saved, co2_reduction)
